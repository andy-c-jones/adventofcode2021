...
public class RoundExecutorTests
{
    ...
    [Test]
    public void In_part_two_should_execute_round_and_check_but_find_no_winner([Values(1, 321, 543, 23)] int roundNumber)
    {
        var roundExecutor = new RoundExecutor();
        ...
        using (Sequence.Create())
        {
            ...
            var (winner, _) = roundExecutor.ExecutePartTwo(roundNumber, boards);
            Assert.That(winner, Is.False);
        }
    }

    [Test]
    public void In_part_two_when_a_board_wins_the_winner_is_returned()
    {
        var roundExecutor = new RoundExecutor();
        ...
        const int roundNumber = 1;
        var (winner, winningBoards) = roundExecutor.ExecutePartTwo(roundNumber, boards);
        Assert.Multiple(() =>
        {
            Assert.That(winner, Is.True);
            var expectedWinningBoards = new[]
            {
                boardThree.Object
            };
            CollectionAssert.AreEquivalent(expectedWinningBoards, winningBoards.ToArray()); 
        });
    }
    
    [Test]
    public void In_part_two_when_many_boards_are_winning_the_those_winners_are_returned()
    {
        var roundExecutor = new RoundExecutor();
    ...
    [Test]
    public void In_part_two_when_all_boards_are_winning_the_all_winners_are_returned()
    {
        var roundExecutor = new RoundExecutor();
        ...
        {
            Assert.That(winner, Is.True);
            CollectionAssert.AreEquivalent(boards, winningBoards.ToArray());
        });
    }
}